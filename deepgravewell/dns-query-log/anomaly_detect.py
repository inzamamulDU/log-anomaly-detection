...
# ================== part2 to transform the textual value to numerical data ================
    ...
    key_para_dict_filename = 'pre_process/key_para_dict.csv'
    # build the dict with index, in order to trace back then
    key_para_dict_index_filename = 'result/key_para_dict_index.csv'
    ...
    tokens_dict_filename = 'tokens_dict.pkl'
...
# ================== part4 to analyse the matrix ===================
    ...
    for file in filenames:
        ...
        if matrix.shape[0] >= 4:
            ...
            for x in yhat:
                ...
                trac_dict[indc].append(x)
            #temp = {file:yhat}
            #trac_dict.append(temp)
            #np.save()
            rmse, means = matrix_analyse_report_anomaly.mean_squared_error_modified(Y, yhat)
    ...
    trac = pd.DataFrame.from_dict(trac_dict,orient='index')
    trac = trac.transpose()
    print(trac)
    
    trac.to_csv('result/RMSE.csv',index = False)
    #df.to_csv("traceback.csv")
